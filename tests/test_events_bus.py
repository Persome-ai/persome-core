...
async def test_publish_from_worker_thread_reaches_subscriber() -> None:
    ...
    # Publish from a worker thread, exactly like a pipeline stage does.
    threading.Thread(
        target=lambda: ev.publish("classifier", "tool_call", {"name": "search_memory"})
    ).start()
    await asyncio.wait_for(task, timeout=3)

    assert got == [{"stage": "classifier", "type": "tool_call", "name": "search_memory"}]


async def test_publish_with_no_subscribers_is_noop() -> None:
    # No subscribers and no init() — must not raise.
    ev.publish("classifier", "tool_call", {"name": "x"})

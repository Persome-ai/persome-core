...
Guidance for Codex (Codex.ai/code) and other coding agents working in this repository.
...
persome status
persome stop
persome mcp                          # stdio MCP transport (for Codex Desktop / Cursor)
```
...
- **LLM calls go through ONE path** — `src/persome/writer/llm.py`, which speaks the **Anthropic
  ...
  (e.g. `Codex-haiku-4-5`, `deepseek-v4-flash`) sent verbatim to the gateway; `anthropic/…` /

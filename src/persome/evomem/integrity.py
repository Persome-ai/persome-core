...
报警通路 = ``logger.error`` + ``events.publish`` 一条 ``integrity_alert`` SSE 事件
（payload 含 check 名 / 详情 / 来源 / 是否冻结），HUD 侧消费不在 PR-1 范围。
"""
...
# violation so a badly damaged DB doesn't flood the log / SSE payload.
...
def emit_alert(
    ...
    *,
    ...
    """logger.error + one ``integrity_alert`` SSE event. The single alert
    channel for self-check failures AND bad-snapshot findings (backup.py)."""
    _log.error(

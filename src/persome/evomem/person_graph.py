...
- **名字来源是可注入 seam**（``PersonNameSource``）：默认从现有 intents 表的
  ``payload.with`` / ``payload.participants`` 读（不确定就空），测试注入假数据。
  **不读/不改 intent recognizer** —— 只消费它已落库的产出。
...
class PersonNameSource(Protocol):
    ...
    默认实现从 intents 表的 ``payload.with`` 读；测试注入假数据。实现方只读现有产出，
    **不得**触发 intent recognizer。
    """
...
class IntentPersonNameSource:
    """从 intents 表的 ``payload.with`` / ``payload.participants`` 提取人名事件。

    只**读** intents（recognizer 已落库的产出），每个 ``with`` 名字 × 每条 intent 产出
    一条 :class:`PersonEvent`，``occurred_at`` 取 intent 的 ``ts``，``summary`` 取
    rationale/kind 兜底，``confidence`` 取 intent.confidence。读不到/解析失败 → 空列表
    （fail-safe，绝不抛）。``conn_factory`` 注入便于测试；默认用 ``store.fts.cursor``。
    """
    ...
    def events(self) -> list[PersonEvent]:  # noqa: D102
        ...
        except Exception:  # noqa: BLE001 — fail-safe：来源读不到绝不阻断/抛
            _log.debug("IntentPersonNameSource: read failed, returning empty", exc_info=True)
            return []

    def _collect(self) -> list[PersonEvent]:
        ...
        with cm as conn:
            rows = conn.execute(
                "SELECT ts, kind, confidence, rationale, payload FROM intents "
                "ORDER BY ts DESC LIMIT ?",
                (self._limit,),
            ).fetchall()
        for row in rows:
            ts, kind, confidence, rationale, payload_text = (
                row[0],
                row[1],
                row[2],
                row[3],
            ...
            try:
                ...
                continue
            people = payload.get("with") or payload.get("participants") or []
            if not isinstance(people, list):
                continue
            occurred = _parse_ts(ts)
            summary = (rationale or "").strip() or f"出现在一次 {kind} 场景"

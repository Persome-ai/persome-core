...
- `drill_chat_captures(app_name, start_ts, end_ts, max_bytes=12000)` — reconstruct a conversation from raw screen captures for a chat app (e.g. `飞书`, `WeChat`, `Messages`). Returns timestamped visible_text snapshots with `⚠️ [gap: fast scroll detected]` markers where content may be missing. **Call this when a session entry shows significant chat app activity and you need the actual conversation to extract durable facts** (decisions, action items, contact roles, project names, recurring topics). Use the session's `start_time` and `end_time` as `start_ts`/`end_ts`. The app_name must match the captured app_name exactly (check the timeline blocks for the exact string).
